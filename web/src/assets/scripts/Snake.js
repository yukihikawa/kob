...
export class Snake extends AcGameObject {
    ...
    next_step(){ //蛇的状态变为走下一步
        ...
        for (let i = k; i > 0; i -- ) { //初始元素不变，每个元素向后移动一位
            //头部多了一个自己的复制
            //使用深拷贝
    ...
    update_move(){
        ...
        if(distance < this.eps){ //到达目标
            this.cells[0] = this.next_cell;  // 添加一个新蛇头
            this.next_cell = null;
            this.status = 'idle';//停下来

            if(!this.check_tail_increasing()){ //蛇不变长，截断蛇尾
                this.cells.pop();
            }
    ...
    render(){
        ...
        for (let i = 1; i < this.cells.length; i++) {
            ...
            if (Math.abs(a.x - b.x) < this.eps && Math.abs(a.y - b.y) < this.eps) //在同一位置（重合
                continue;
